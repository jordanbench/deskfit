...
const text = fs.existsSync(reportPath)
  ...
  : "Deskfit weekly pulse could not find the local report.";
const html = renderAdminPulseHtml({
  brand: "Deskfit",
  siteUrl: "https://www.deskfit.co",
  accent: "#2563eb",
  text,
  testMode: isTest,
});
...
const response = await fetch("https://api.resend.com/emails", {
  ...
  body: JSON.stringify({ from, to, subject: `${isTest ? "[TEST] " : ""}Deskfit weekly affiliate pulse`, text, html })
});
...
function renderAdminPulseHtml({ brand, siteUrl, accent, text, testMode }) {
  ...
  const releaseResult = parsed.sections.get("Release result") || [];
        ...
        <table role="presentation" width="680" cellpadding="0" cellspacing="0" style="width:680px;max-width:100%;background:#fbfaf6;border:1px solid #d9ded8;border-radius:10px;overflow:hidden;">
          ...
          <tr>
            <td style="padding:24px 30px;">
              ...
              ${renderSection("Release result", releaseResult, "priority")}
