...
const envStatus = {
  ...
  vercelOrgId: Boolean(process.env.VERCEL_ORG_ID),
...
const report = [
  ...
  `- Resend credentials available: ${envStatus.resend ? "yes" : "no"}`,
  `- Supabase URL available: ${envStatus.supabaseUrl ? "yes" : "no"}`,
  `- Supabase service role key available: ${envStatus.supabaseServiceRoleKey ? "yes" : "no"}`,
  `- Vercel credentials available: ${envStatus.vercelToken && envStatus.vercelOrgId && envStatus.vercelProjectId ? "yes" : "no"}`,
].join("\n");
